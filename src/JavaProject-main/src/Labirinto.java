...
public class Labirinto {
    ...
    public Labirinto(Aventureiro jogador, int dificuldade) {
        ...
        this.tesouros = (List<Tesouro>) config.getTesouros();  // Agora as listas de tesouros e perigos são configuradas aqui
        this.perigos = config.getPerigos();
        this.salaSecretaDesbloqueada = false;
        this.inimigos = config.getInimigos(); // <-- ADICIONE ESTA LINHA
    }
    ...
    for (int i = 0; i < mapa.length; i++) {
        for (int j = 0; j < mapa[i].length; j++) {
            if (mapa[i][j].equals("J") || mapa[i][j].equals("I")) {
                mapa[i][j] = ".";
            }
        }
    }
    ...
    // Atualiza inimigos (só se não for a posição do jogador)
    int[] posJogador = jogador.getLocalizacao();
    for (Inimigo inimigo : getInimigos()) {
        if (inimigo.getVida() > 0) {
            int[] pos = inimigo.getLocalizacao();
            if (!(pos[0] == posJogador[0] && pos[1] == posJogador[1])) {
                mapa[pos[0]][pos[1]] = "I";
            }
        }
    }

    // Coloca o jogador por último
